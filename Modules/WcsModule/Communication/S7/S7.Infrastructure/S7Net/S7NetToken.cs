...
using S7.Net;
using EnumHelper = S7.Infrastructure.Helper.EnumHelper;
...
public class S7NetToken : INet
{
    ...
    public S7NetToken(NetModel netConfig, Action<string> logAciont = null)
    {
        ...
        Plc = new Plc(EnumHelper.S7CpuTypeToVarType(netConfig.S7Type), netConfig.Ip, netConfig.Port, netConfig.Rack,
            netConfig.Solt);
    ...
    public async Task<Result> ConnectAsync()
    {
        ...
        for (var i = 0; i < _netConfig.MaxRetries; i++)
            try
            {
                await Plc.OpenAsync();
        ...
        if (input is ReadModel readModel)
            try
            {
                var bufferBlock = await
                    Plc.ReadBytesAsync(EnumHelper.S7BlockTypeToDataType(readModel.S7BlockType)
    ...
    public async Task<Result> WriteAsync(IWriteConfig input)
    {
        ...
        if (input is WriteModel writeModel)
        {
            ...
            {
                await Plc.WriteBytesAsync(EnumHelper.S7BlockTypeToDataType(writeModel.S7BlockType),
                    writeModel.DBAddress, writeModel.DBStart, writeModel.Buffer);
                var buffer = Plc.ReadBytes(EnumHelper.S7BlockTypeToDataType(writeModel.S7BlockType),
                    writeModel.DBAddress, writeModel.DBStart, writeModel.Buffer.Length);
                if (SequenceEquals.ByteSequenceEquals(buffer, writeModel.Buffer)) result = Result.Success();
            }
            else
            {
                await Plc.WriteBitAsync(EnumHelper.S7BlockTypeToDataType(writeModel.S7BlockType), writeModel.DBAddress,
                    writeModel.DBStart, writeModel.BitAddress.Value,
                    writeModel.BitValue.Value);
                var @bool = Plc.Read(EnumHelper.S7BlockTypeToDataType(writeModel.S7BlockType), writeModel.DBAddress,
                    writeModel.DBStart, VarType.Bit, writeModel.BitAddress.Value);
