...
public static class DecoratorConfiguration
{
    ...
    /// <returns></returns>
    public static IServiceCollection AddDecoratorConfiguration(this IServiceCollection serviceCollection, Type[] decorateTypes,
        params Assembly[] assArray)
    {
        serviceCollection.Scan(scan => scan.FromAssemblies(assArray)
            .AddClasses(classes => classes.AssignableTo(typeof(IDomainEventHandler<>)))
            ...
            .WithTransientLifetime());
        // 2. 用幂等装饰器包装它们
        foreach (var item in decorateTypes)
        {
            serviceCollection.Decorate(typeof(IDomainEventHandler<>), item);
        }
        serviceCollection.AddScoped<IDomainEventDispatcher, MediatrDomainEventDispatcher>();
        return serviceCollection;
    }
