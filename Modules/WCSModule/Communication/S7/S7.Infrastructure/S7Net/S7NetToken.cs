...
public class S7NetToken : INet
{
    ...
    public S7NetToken(NetModel netConfig, Action<string> logAciont = null)
    {
        ...
        Plc = new S7.Net.Plc(EnumHelper.S7CpuTypeToVarType(netConfig.S7Type), netConfig.Ip, netConfig.Port, netConfig.Rack, netConfig.Solt);
    ...
    public async Task<Result> WriteAsync(IWriteConfig input)
    {
        ...
        if (input is WriteModel writeModel)
        {
            ...
            {
                await Plc.WriteBytesAsync(EnumHelper.S7BlockTypeToDataType(writeModel.S7BlockType), writeModel.DBAddress, writeModel.DBStart, writeModel.Buffer);
                var buffer = Plc.ReadBytes(EnumHelper.S7BlockTypeToDataType(writeModel.S7BlockType), writeModel.DBAddress, writeModel.DBStart, writeModel.Buffer.Length);
                if (Common.Helper.SequenceEquals.ByteSequenceEquals(buffer, writeModel.Buffer))
                {
                    result = Result.Success();
                }
            }
            ...
            {
               await Plc.WriteBitAsync(EnumHelper.S7BlockTypeToDataType(writeModel.S7BlockType), writeModel.DBAddress, writeModel.DBStart, writeModel.BitAddress.Value,
                    writeModel.BitValue.Value);
                var @bool = Plc.Read(EnumHelper.S7BlockTypeToDataType(writeModel.S7BlockType), writeModel.DBAddress, writeModel.DBStart, VarType.Bit, writeModel.BitAddress.Value);
